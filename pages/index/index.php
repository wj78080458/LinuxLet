...
<head>
    ...
    <script type="text/javascript">
    	...
    	function btnClick(){
	        ...
	        if ($('#right_layer').hasClass('rightDiv')) {
		        // Close Nav
		        $('#right_layer').removeClass('rightDiv');
		    
		    } else {
		        // Open Nav
		        $('#right_layer').addClass('rightDiv');
...
<body>
	...
	<?PHP
		...
		function get_item_data($tag){
			...
			$itemData->showLevel = $tag->getElementsByTagName("showLevel")->item(0)->nodeValue;
			$itemData->config = $tag->getElementsByTagName("config")->item(0)->nodeValue;
			$itemData->category = $tag->getElementsByTagName("category")->item(0)->nodeValue;
    ...
    <div  id="right">
    	<?php
    		if($pageType == 0){
    			...
    			<input class="topFindImg" type="image" name="searchId" value="1" title="搜索手册" alt="搜索手册" src="../../images/search_global.jpg" onclick ="document.foormName.submit()"/>
   				</form>';
   				...
   				if($pageType != 6){
   					echo '<div class="titleDiv">','当前位置：','<a href="index.php?&foldId=',$foldId,'&subId=0">',$firstName,'</a>','->','<a href="index.php?&foldId=',$foldId,'&subId=',$subId,'">',$secondName,'</a>';
   					if($thirdName != null){
   						echo '->',$thirdName;
   						if($forthName != null){
   							echo '->',$forthName;
	    		...
	    		if($pageType == 1){
	    			...
	    			//具体内容，使用iframe显示
	    			echo '<iframe class="mainConDiv" frameborder="0" src="../../files/',$firstName,'/',$secondName,'/',$thirdName,'/',$forthName,'/',$forthName,'.html','"/>';
	    		}else if($pageType == 6){
